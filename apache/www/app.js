...
function initializeDdRum() {
  if (
    !window.DD_RUM ||
    !ddRumConfig ||
    !ddRumConfig.applicationId ||
    !ddRumConfig.clientToken ||
    window.__ddRumInitialized
  ) {
    return;
  }
...
initializeDdRum();
window.addEventListener("dd-rum-ready", initializeDdRum);
